...
-- Looking at highest Infection Rate compared to Population per Country:
...
-- Looking at highest Infection Rate compared to Population per Country DAILY:
...
-- Total Numbers:
SELECT SUM(new_cases) as TotalCases, SUM(cast(new_deaths AS SIGNED)) as TotalDeaths, SUM(cast(new_deaths AS SIGNED))/SUM(new_cases)*100 as GlobalDeathPercentage
FROM covidporfolioproject.coviddeaths
WHERE continent is not null
ORDER BY 1,2;
...
-- Daily Infection Rate VS Population per Country:
CREATE VIEW DailyPopInfection AS
SELECT location, population, date, MAX(total_cases) AS HighestInfectionCount, MAX(total_cases/population)*100 AS InfectedPopulationPercentage
FROM covidporfolioproject.coviddeaths
GROUP BY location, population, date
ORDER BY InfectedPopulationPercentage desc;
